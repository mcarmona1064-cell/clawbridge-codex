...
export function upsertMemory(memory: Memory): void {
  ...
  db.prepare(`
    INSERT OR REPLACE INTO memories
      (id, client_id, segment, content, importance, decay_rate, created_at, last_accessed_at, expires_at)
    VALUES
      (@id, @clientId, @segment, @content, @importance, @decayRate, @createdAt, @lastAccessedAt, @expiresAt)
  `).run({
    id: memory.id,
...
export function getMemories(clientId: string, minImportance = 0.3): Memory[] {
  ...
  const rows = db
    .prepare(`SELECT * FROM memories WHERE client_id = ?`)
    .all(clientId) as MemoryRow[]
