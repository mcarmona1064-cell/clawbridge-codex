---
name: x-integration
description: X (Twitter) integration for ClawBridge. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** ClawBridge v1.0.0. Directory structure may change in future versions.
...
1. **ClawBridge is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep clawbridge (macOS) or systemctl --user status clawbridge (Linux)
```
...
| `CLAWBRIDGE_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/clawbridge.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into ClawBridge, make the following modifications:
...
All paths below are relative to project root (`CLAWBRIDGE_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep clawbridge  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status clawbridge
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/clawbridge.log | tail -20

# Script errors
grep -i "error\|failed" logs/clawbridge.log | tail -20
```
...
docker run clawbridge-agent ls -la /app/src/skills/
```
