...
Connect ClawBridge to Microsoft Teams for interactive chat in team channels, group chats, and direct messages.

## Install

ClawBridge doesn't ship channels in trunk. This skill copies the Teams adapter in from the `channels` branch.
...
2. Click **New client secret**, description "clawbridge", expiry 180 days
...
```bash
az group create --name clawbridge-rg --location eastus
az bot create \
...
```bash
az bot msteams create --resource-group clawbridge-rg --name clawbridge-bot
```
...
```json
{
  ...
  "name": { "short": "ClawBridge", "full": "ClawBridge Assistant" },
  "description": {
    "short": "ClawBridge assistant bot",
    "full": "ClawBridge personal assistant powered by Claude."
  },
...
- **how-to-find-id**: Send a message to the bot in the channel. ClawBridge auto-creates a messaging group and logs the platform ID. Use that messaging group ID for wiring.
