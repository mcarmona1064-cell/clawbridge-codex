# Migrating OpenClaw Cron Jobs to ClawBridge Scheduled Tasks
...
## ClawBridge `scheduled_tasks` Table
...
- `delivery.mode:"webhook"` — ClawBridge has no webhook delivery. Discuss with the user: this could be implemented as a task `script` that runs `curl` to hit the webhook endpoint.
- `failureAlert` — ClawBridge has no failure alert system. Note this to the user.
- `wakeMode` — ClawBridge tasks always wake the agent immediately.
- `payload.model`, `payload.thinking`, `payload.timeoutSeconds` — ClawBridge doesn't support per-task model/thinking config. These are handled by the SDK.
- `deleteAfterRun` — ClawBridge `"once"` tasks are marked `"completed"` after running, not deleted.
