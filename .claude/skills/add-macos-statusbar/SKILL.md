...
description: Add a macOS menu bar status indicator for ClawBridge. Shows a bolt icon with a green/red dot indicating whether ClawBridge is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows ClawBridge's running status and lets the user
...
```bash
launchctl list | grep com.clawbridge.statusbar
```
...
Create `~/Library/LaunchAgents/com.clawbridge.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.clawbridge.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.clawbridge.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see ClawBridge's status and control the service.
>
> - **Green dot** — ClawBridge is running
> - **Red dot** — ClawBridge is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.clawbridge.statusbar.plist
rm ~/Library/LaunchAgents/com.clawbridge.statusbar.plist
rm dist/statusbar
```
