...
Adds Parallel AI MCP integration to ClawBridge for advanced web research capabilities.
...
- **Non-blocking Design** - Uses ClawBridge scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. ClawBridge already set up and running
...
Add Parallel AI MCP servers after the clawbridge server:
```typescript
const mcpServers: Record<string, any> = {
  clawbridge: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__clawbridge__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__clawbridge__send_message
   4. Use mcp__clawbridge__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo clawbridge-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep clawbridge  # macOS
...
- Check task runs: `tail -f logs/clawbridge.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.clawbridge` (macOS) or `systemctl --user restart clawbridge` (Linux)
