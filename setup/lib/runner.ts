 ...
 *
 * Responsibilities:
 *   - Stream-parse setup-step status blocks (`=== CLAWBRIDGE SETUP: … ===`)
...
/**
 * Streaming parser for `=== CLAWBRIDGE SETUP: TYPE ===` blocks. Emits each
...
export class StatusStream {
  ...
  private processLine(line: string): void {
    const start = line.match(/^=== CLAWBRIDGE SETUP: (\S+) ===/);
    if (start) {
  ...
  return new Promise((resolve) => {
    ...
    const pushLines = (chunk: string): void => {
      ...
      while ((idx = lineBuf.indexOf('\n')) !== -1) {
        ...
        if (line.startsWith('=== CLAWBRIDGE SETUP:')) continue;
...
export function dumpTranscriptOnFailure(transcript: string): void {
  const lines = transcript.split('\n').filter((l) => {
    if (l.startsWith('=== CLAWBRIDGE SETUP:')) return false;
...
export async function fail(
  ...
  if (ranFix) {
    ...
    if (retry) {
      const existingSkip = (process.env.CLAWBRIDGE_SKIP ?? '')
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
      const skipList = [
      ...
      const result = spawnSync('pnpm', ['--silent', 'run', 'setup:auto'], {
        stdio: 'inherit',
        env: { ...process.env, CLAWBRIDGE_SKIP: skipList },
      });
