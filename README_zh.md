...
<p align="center">
  ...
  <a href="https://docs.clawbridge.dev">文档</a>&nbsp; • &nbsp;
...
ClawBridge 用一个您能轻松理解的代码库提供了同样的核心功能：一个进程，少数几个文件。Codex 智能体运行在具有文件系统隔离的独立 Linux 容器中，而不是仅靠权限检查。
...
`clawbridge.sh` 会把您从一台全新机器一直带到一个可以直接发消息的命名智能体。它会在缺失时安装 Node、pnpm 和 Docker，将 OpenAI 凭据保存到 `.env`，构建智能体容器，并配对您的第一个渠道（Telegram、Discord、WhatsApp 或本地 CLI）。如果某一步失败，会自动调用 Codex 进行诊断并从中断处继续。

## 设计哲学

**小到可以理解。** 单一进程，少量源文件，无微服务。如果您想了解完整的 ClawBridge 代码库，直接让 Codex 给您讲一遍就行。

**通过隔离实现安全。** 智能体运行在 Linux 容器中，只能看到明确挂载的内容。Bash 访问是安全的，因为命令在容器内执行，而不是在您的宿主机上。

**为个人用户打造。** ClawBridge 不是一个单体框架，而是能精确匹配每个用户需求的软件。它被设计成量身定制的，而不是臃肿膨胀。您创建自己的 fork，让 Codex 按您的需求修改它。

**定制 = 修改代码。** 没有配置膨胀。想要不同的行为？改代码。代码库小到改动是安全的。

**AI 原生，混合式设计。** 安装与上手流程走的是经过优化的脚本路径，快速且确定。当某一步需要判断（安装失败、引导决策、定制化）时，控制权会无缝地交给 Codex。安装之后也不提供监控仪表盘或调试 UI：您在聊天中描述问题，Codex 来处理。

**技能优于功能。** 主干只发布注册表和基础设施，不包含具体的渠道适配器或替代智能体提供者。各个渠道（Discord、Slack、Telegram、WhatsApp……）放在长期存在的 `channels` 分支上；Ollama 放在 `providers` 分支上。您运行 `/add-telegram` 等，技能会把您所需要的模块精确地复制到您的 fork 里。不会出现您没要求的功能。

**最强的 harness，最强的模型。** ClawBridge 通过 OpenAI 官方的 Codex CLI 原生使用 Codex，所以您能用上最新的 Codex 模型以及 Codex 的完整工具集——包括修改和扩展自己的 ClawBridge fork 的能力。如需本地开源权重模型，使用 `/add-ollama-provider`。
...
- **凭据安全** — 智能体不持有原始 API key。凭据在容器启动时直接从 `~/.clawbridge/.env` 注入，不通过代理传递。
...
ClawBridge 不用配置文件。想改就直接告诉 Codex：
...
- [Codex](https://claude.ai/download)，用于 `/customize`、`/debug`、安装过程中的错误恢复以及所有 `/add-<channel>` 技能

## 架构

```
消息应用 → 主机进程（路由器） → inbound.db → 容器（Bun、Codex CLI） → outbound.db → 主机进程（投递） → 消息应用
```

单一 Node 主机编排每个会话的智能体容器。当一条消息到来时，主机按实体模型（用户 → 消息组 → 智能体组 → 会话）进行路由，写入该会话的 `inbound.db`，并唤醒容器。容器内部的 agent-runner 轮询 `inbound.db`，调用 Codex，并把响应写入 `outbound.db`。主机轮询 `outbound.db`，通过渠道适配器投递回去。
...
- `src/container-runner.ts` — 为每个智能体组启动容器，从 .env 注入凭据
...
可以。Docker 是默认运行时，可在 macOS、Linux 以及 Windows（通过 WSL2）上工作。运行 `bash clawbridge.sh` 就行。

**这个项目安全吗？**

智能体运行在容器里，而不是躲在应用级权限检查之后。它们只能访问明确挂载的目录。凭据不会直接暴露给容器——在容器启动时从 `~/.clawbridge/.env` 注入，且宿主机的 `.env` 文件不会挂载到容器内。您仍然应该审查自己要运行的代码，但代码库小到您真的能做到。完整的安全模型见 [安全文档](https://docs.clawbridge.dev/concepts/security)。

**为什么没有配置文件？**

我们不想让配置泛滥。每位用户都应该定制 ClawBridge，让代码精确地做他们想要的事，而不是去配置一个通用系统。如果您更喜欢有配置文件，可以让 Codex 给您加。

**我可以使用第三方或开源模型吗？**

可以。使用 `/add-ollama-provider` 通过 Ollama 运行本地开源权重模型。

对于一次性实验，任何 Codex API 兼容的端点也可以通过 `.env` 使用：
...
问 Codex。"为什么计划任务没运行？""最近的日志里有什么？""为什么这条消息没有得到回复？"这就是 ClawBridge 底层的 AI 原生方式。

**为什么安装对我不成功？**

如果某一步失败，`clawbridge.sh` 会把控制权交给 Codex 进行诊断并从中断处继续。如果还是没解决，运行 `claude`，然后 `/debug`。如果 Codex 发现一个可能影响其他用户的问题，请对相关的安装步骤或技能提 PR。
...
破坏性变更见 [CHANGELOG.md](CHANGELOG.md)，完整发布历史见文档站的 [full release history](https://docs.clawbridge.dev/changelog)。
