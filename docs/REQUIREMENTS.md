...
ClawBridge gives you the core functionality without that mess.
...
> **Note:** Telegram, Slack, Discord, Gmail, and Apple Container skills already exist. See the [skills documentation](https://docs.clawbridge.dev/integrations/skills-system) for the full list.
...
- Use existing tools (channel libraries, Codex CLI, MCP servers)
- Minimal glue code
...
- Custom `clawbridge` MCP server (inside container) provides scheduling tools
...
- `/update-clawbridge` - Pull upstream changes, merge with customizations
...
**ClawBridge** - A reference to Clawdbot (now OpenClaw).
