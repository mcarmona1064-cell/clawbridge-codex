...
user clones upstream main
  ├── merges clawbridge-whatsapp fork  → adds WhatsApp
...
```bash
# In your local clawbridge checkout
git checkout main && git pull

# For a fork:
git fetch clawbridge-whatsapp
git checkout -B whatsapp-merge clawbridge-whatsapp/main
git merge main
...
git push clawbridge-whatsapp HEAD:main
...
1. Fork `clawbridge` to `clawbridge-{channel}`
