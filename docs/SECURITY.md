...
**External Allowlist** - Mount permissions stored at `~/.config/clawbridge/mount-allowlist.json`, which is:
...
ClawBridge uses pnpm with two supply chain defenses configured in `pnpm-workspace.yaml`:
