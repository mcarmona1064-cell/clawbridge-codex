...
You also have `mcp__clawbridge__send_message` which sends a message immediately while you're still working. This is useful when you want to acknowledge a request before starting longer work.
...
If the user wants to set up an allowlist, edit `~/.config/clawbridge/sender-allowlist.json` on the host:
...
- The config file is on the host at `~/.config/clawbridge/sender-allowlist.json`, not inside the container
